...
class Bucket:   # 해시를 구성하는 버킷
    def __init__(self, key: Any = None, value: Any = None,
                 stat: Status = Status.EMPTY) -> None:      # 초기화
        self.key = key   # 키
        self.value = value # 값
        self.stat = stat # 속성
    ...
    def set_status(self, stat: Status) -> None:  # 속성을 설정
        self.stat = stat
...
class OpenHash:     # 오픈 주소법으로 구현하는 해시 클래스
    ...
    def rehash_value(self, key: Any) -> int:  # 재해시값을 구함
        return(self.hash_value(key) + 1) % self.capacity
    
    def search_node(self, key: Any) -> Any:  # 키가 key인 버킷을 검색
    ...
    def remove(self, key: Any) -> int:   # 키가 key인 원소를 삭제
